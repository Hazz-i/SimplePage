    ...
    <div className="flex flex-col mb-14">
      ...
      <div className="flex items-center justify-center flex-wrap my-10 gap-10">
        {books.map((book, index) => (
          <span className="w-[36rem] h-[20rem] border shadow-lg flex" key={index} data-aos="fade-in">
            <div className={`w-1/2 border h-full ${index > 3 ? "order-0" : "order-1"} overflow-hidden flex items-center justify-center`}>
              <img src={`${book.img}`} alt="" />
            </div>
            <div className={`w-1/2 bg-icons p-5 flex flex-col gap-2 ${index > 2 ? "order-1" : "order-0"}`}>
